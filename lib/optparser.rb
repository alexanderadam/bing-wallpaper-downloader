...
module BingWallpaperDownloader
  ...
  class OptParser
    ...
    def parse_args(args)
      ...
      OptionParser.new do |opts|
        ...
        opts.on('-d',
                ...
                'The directory to save the wallpapers to.') do |directory|
          options[:destination] = File.expand_path(directory)
        end

        opts.on('-l',
                '--locale [LOCALE]',
                "The locale to use when looking for an image. Default: #{options[:locale]}") do |locale|
          options[:locale] = locale
        end

        opts.on('-r',
                '--resolution [RESOLUTION]',
                "The resolution of image to fetch. Default: #{options[:resolution]}") do |resolution|
          options[:resolution] = resolution
        end

        opts.on('-t',
                '--total [TOTAL]',
                "The total number of days back to get images from. Default #{options[:total]} " \
                "Maximum: #{MAX_DAYS_BACK}") do |total|
    ...
    def validate_options(options)
      ...
      if Dir.exist?(options[:destination]) && !File.writable?(options[:destination])
        raise OptionParser::InvalidArgument, "The destination directory '#{options[:destination]}' is not writeable"
      elsif !Dir.exist?(options[:destination]) && !FileUtils.mkdir_p(options[:destination])
      ...
      return if /^\d+x\d+$/.match(options[:resolution])
